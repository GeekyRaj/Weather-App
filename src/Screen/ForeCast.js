...
class Home extends Component {
  ...
  render() {
    const place =this.props.navigation.getParam('data', 'No data');
    const cityname = this.props.navigation.getParam('name', 'No data');
    console.log(place)
    const { text, textdata, } = Styles;

    const API = this.props.weather;
    const currently = place.currently;
    //const temp = (currently.temperature -32)*5/9;
    const dailyData = place.daily.data;
    ...
    return (
      <View style={{ flex: 1, }}>
        ...
        <View style={{ alignItems: 'center', justifyContent: 'center',marginTop:20 }}>
          <Text style={{ fontSize: 30, fontWeight: 'bold' }}>{cityname}</Text>
